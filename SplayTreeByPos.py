...
class Node(object):
    ...
    def isLeftOfParent(self):
        return self.p.l == self
    def isRightOfParent(self):
        return self.p.r == self
...
class SplayTree(object):
    ...
    def doubleRotate(self, node):
        if node.isLeftOfParent() and node.p.isLeftOfParent():
            assert node.p.l == node and node.p.p.l == node.p
            #print "zigzigleft"
            self.zigZigLeft(node)
            return
        if node.isLeftOfParent() and node.p.isRightOfParent():
            assert node.p.l == node and node.p.p.r == node.p
            #print "zigzagleft"
            self.zigZagLeft(node)
            return
        if node.isRightOfParent() and node.p.isRightOfParent():
            assert node.p.r == node and node.p.p.r == node.p
            #print "zigzigRight"
            self.zigZigRight(node)
            return
        if node.isRightOfParent() and node.p.isLeftOfParent():
            assert node.p.r == node and node.p.p.l == node.p
            #print "zigzagRight"
